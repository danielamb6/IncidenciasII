...
function getHeaders() {
    ...
    return {
        ...
        'X-User-Empresa': sesion.id_empresa || '' // Este ID filtra CODIVERSA, SITRAQ, etc.
    };
}
...
// Añadir esta función en Auth.js o en la inicialización de cada página
function aplicarRestriccionesNavegacion() {
    const sesionStr = localStorage.getItem('sesion');
    if (!sesionStr) return;

    const sesion = JSON.parse(sesionStr);
    
    if (sesion.rol === 'emp_admin') {
        ...
        menuLinks.forEach(link => {
            ...
            // Lista de páginas prohibidas para emp_admin
            ...
            if (paginasProhibidas.some(pagina => href.includes(pagina))) {
                // Ocultamos el elemento padre (el li)
                link.parentElement.style.display = 'none';
            }
        });

        // Redirección de seguridad: si el usuario intenta entrar por URL directa
        const pathActual = window.location.pathname;
        if (['crear-ticket', 'registro_usuario', 'clientes', 'tecnicos', 'catalogos'].some(p => pathActual.includes(p))) {
            window.location.href = 'index.html';
        }
    }
}
// Agrega esta función al inicio de Auth.js
function filtrarMenuPorRol() {
    ...
    // Si el usuario es administrador de empresa (CODIVERSA, SITRAQ, etc.)
    if (sesion.rol === 'emp_admin') {
        const paginasProhibidas = [
            'crear-ticket.html',
            'registro_usuario.html',
        ...
        menuLinks.forEach(link => {
            const href = link.getAttribute('href');
            // Si el href coincide con una página prohibida, ocultamos el elemento <li>
            if (paginasProhibidas.some(p => href.includes(p))) {
                const listItem = link.closest('.menu-item') || link.parentElement;
...
// Asegúrate de que se llame en la inicialización principal
document.addEventListener('DOMContentLoaded', function() {
    // ... tu código existente ...
    filtrarMenuPorRol(); // <-- LLAMADA CRUCIAL
    configurarNavegacion();
});

// Ejecutar al cargar el DOM
document.addEventListener('DOMContentLoaded', aplicarRestriccionesNavegacion);
